...
export default {
  ...
  setup() {
    ...
    const saveEntries = async () => {
      const cleanedRows = rowData.value.map((r) => ({
        ...r,
        location:
          r.location && String(r.location).trim() !== ""
            ...
            : null,
        store_location:
          r.store_location && String(r.store_location).trim() !== ""
            ? String(r.store_location).trim()
            : null, // ✅ normalize new field
      }));
      ...
      const invalidRow = cleanedRows.find((r) => {
        ...
        if (Object.values(isInvalid).some((v) => v)) {
          return true;
        }
        return false;
      });
